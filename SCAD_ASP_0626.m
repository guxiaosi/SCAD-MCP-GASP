function SCAD_ASP_0626()
    ...
    for t=1:maxIter
        ...
        %tic;
        for i = 1:N %eqs. 49-51
            [phi(i), dphi_dB(i), delta0(i), delta1(i),tmp_validZ] = compute_phi_scad(A1, B(i), a, A0, m, lam);
            if ~tmp_validZ
                validZ = false;
                break;
            end
        ...
        if (norm(hatx - hatx_old) / norm(hatx))^2 < eps_tol || ~validZ, break; end
...
%% ----------------------------- LOCAL FUNCTION -----------------------------
function [phi, dphi_dB, delta0, delta1, validZ] = compute_phi_scad(A1, B, a, A0, m, lam)
